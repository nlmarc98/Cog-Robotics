  ...
  {body: null,  // for MatterJS body, added by InstantiateRobot()
   ...
   sensors: [  // define an array of sensors on the robot
     ...
     {sense: senseDistance, minVal: 0, maxVal: 50, attachAngle: Math.PI/30, 
      attachRadius: 15, lookAngle: 0, id: 'frontS_dist_all', color: [0, 0, 0, 50], 
     ...
     {sense: senseColor, minVal: 0, maxVal: 15, attachAngle: -0.5585993153435624 ,
      attachRadius: 5, lookAngle: 70.753, id: 'color_gripper', color: [0, 255, 0, 50],
      parent: null, value: null, valueStr: ''
     },
	 // define one color sensor
	 {sense: senseColor, minVal: 0, maxVal: 50, attachAngle: Math.PI/30,
      attachRadius: 15, lookAngle: 0, id: 'frontS_color_all', color: [0, 255, 0, 50],
      parent: null, value: null, valueStr: ''
     }
   ]
  }, 
...
simInfo = {
  numRobots: 3, // number of robots in the simulator.
  ...
  boxFric: 0.008, // friction between boxes during collisions
...
/* Add a bunch of boxes in a neat grid. */
  ...
  function getBox(x, y) {
	...
	// When nmbr is even the next box that will be created is red.
	// When nmbr is odd the next box that will be created is blue.
    nmbr = nmbr+1;  
    if (nmbr % 2 == 0){
      color = [200, 0, 0];
    }
 ...
 */
  ...
  if (frontS_color_all == "r" || frontS_color_all == "b"){
	  if (color_gripper == "n") {
		  ...
		  robot.rotate(robot, 0.3*defaultRotate);
	  }
	  else if (color_gripper == "r") {
		  robot.rotate(robot, -45*defaultRotate);
	  }
  }
  else if (frontS_color_all == "w" && frontS_dist_all < 7) {
	  robot.drive(robot,-0.0005);
	  if (color_gripper == "n") {
		  robot.rotate(robot,-180*defaultRotate);
	  }
	  else if (color_gripper == "b" ){
		  robot.rotate(robot, -10*defaultRotate);
	  }
	  else if (color_gripper == "r" ){
		  robot.rotate(robot,30*defaultRotate);
	  }
  }
