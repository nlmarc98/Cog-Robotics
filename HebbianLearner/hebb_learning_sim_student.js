 ...
 *
 *
 * Based on the Didabots - robot and GUI script.
 *
...
RobotInfo = [
  ...
  {body: null, color: "red", init: {x: 50, y: 400, angle: 5.5},
	...
	// Starting weights. 
	weights_dist: [0.01, 0.01, 0.01],
	weights_touch:[0.5, 0.5, 0.5]
  },
  // Second Robot
...
function loadBay(robot) {
  ...
  for (var ss = 0; ss < robot.info.sensors.length; ++ss) {
    ...
    sensors[ss] = makeInteractiveElement(new SensorGraphics(curSensor),
                                         document.getElementById("bayHebbbot"));
...
function InstantiateRobot(robotInfo) {
  ...
  this.weights_dist = robotInfo.weights_dist;
  this.weights_touch = robotInfo.weights_touch;
...
// This function is called each timestep and should be used to move the robots
	...
	var activationM = activation(outM, threshold);
	var activationR = activation(outR, threshold);
	
	robot.weights_dist[0] = robot.weights_dist[0] + learningrate * activationL * distL_r;
	robot.weights_dist[1] = robot.weights_dist[1] + learningrate * activationM * distM_r;
	robot.weights_dist[2] = robot.weights_dist[2] + learningrate * activationR * distL_r;
...
function plotRobot(context,
  ...
  if (context.canvas.id == "bayHebbbot") {
  ...
  else { // context.canvas.id == "arenaHebbbot"
  ...
  if (context.canvas.id == "arenaHebbbot") {
...
function drawBoard() {
  var context = document.getElementById('arenaHebbbot').getContext('2d');
...
function setRobotNumber(newValue) {
  ...
  while (robots.length < newValue) {
    ...
    robots[n] = makeInteractiveElement(new InstantiateRobot(RobotInfo[n]),
                                       document.getElementById("arenaHebbbot"));
